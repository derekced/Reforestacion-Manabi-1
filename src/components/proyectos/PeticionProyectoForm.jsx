...
export default function PeticionProyectoForm({ onSubmit }) {
  ...
  const validate = () => {
    const e = {};
    if (!form.nombre.trim()) e.nombre = "Nombre del proyecto requerido";
    if (!form.ubicacion.trim()) e.ubicacion = "Ubicación requerida";
    if (!form.lat || isNaN(Number(form.lat))) e.lat = "Latitud válida requerida";
    if (!form.lng || isNaN(Number(form.lng))) e.lng = "Longitud válida requerida";
    if (!form.fecha) e.fecha = "Fecha requerida";
    if (!form.arboles || isNaN(Number(form.arboles)) || Number(form.arboles) <= 0) e.arboles = "Cantidad válida de árboles";
    if (!form.voluntarios || isNaN(Number(form.voluntarios)) || Number(form.voluntarios) <= 0) e.voluntarios = "Cantidad válida de voluntarios";
    if (!form.especies.trim()) e.especies = "Especies requeridas (separadas por coma)";
  ...
  if (success) return (
    <div className="p-6 text-center">
      ...
      <h2 className="text-2xl font-bold mb-2">¡Petición enviada!</h2>
      <p className="text-gray-700">Tu solicitud será revisada por el administrador. Recibirás notificación cuando sea aceptada.</p>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 p-6">
      <div>
        ...
        <input name="nombre" value={form.nombre} onChange={handleChange} className={`w-full px-4 py-2 border rounded-lg ${errors.nombre ? 'border-red-500' : 'border-gray-300'}`} placeholder="Ej: Bosque Urbano Manta" />
        {errors.nombre && <p className="text-red-600 text-sm mt-1">{errors.nombre}</p>}
      </div>
      <div>
        <label className="block font-medium mb-1">Ubicación *</label>
        <input name="ubicacion" value={form.ubicacion} onChange={handleChange} className={`w-full px-4 py-2 border rounded-lg ${errors.ubicacion ? 'border-red-500' : 'border-gray-300'}`} placeholder="Ej: Manta" />
        {errors.ubicacion && <p className="text-red-600 text-sm mt-1">{errors.ubicacion}</p>}
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block font-medium mb-1">Latitud *</label>
        ...
        <div>
          <label className="block font-medium mb-1">Longitud *</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block font-medium mb-1">Árboles *</label>
        ...
        <div>
          <label className="block font-medium mb-1">Voluntarios *</label>
      ...
      <div>
        <label className="block font-medium mb-1">Especies (separadas por coma) *</label>
      ...
      <div>
        ...
        <textarea name="descripcion" value={form.descripcion} onChange={handleChange} rows={3} className={`w-full px-4 py-2 border rounded-lg ${errors.descripcion ? 'border-red-500' : 'border-gray-300'}`} placeholder="Describe el objetivo y actividades del proyecto" />
      ...
      <button type="submit" className="w-full py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg mt-4 transition-colors">Enviar Petición</button>
    </form>
  );
}
